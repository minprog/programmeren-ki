...
- an `import` statement
- an `Adventure` class
- an `if _ \_name_ _ == "_ \_main_ _"` part
...
The `_ \_init_ _` method initializes each room with a given id, name and description.
...
Before you jump in and start programming, realize that you are going to be working on a big program. Consisting of multiple files, multiple ideas and a whole bunch of features. Chances are that you'll find yourself backtracking to code you have written earlier in the assignment as we introduce new features and functionality to the game. You might then have to rewrite pieces of your own code once you find it does not fit your current usecase. This is okay and intentional!
...
We call the `load_rooms` method in the `_ \_init_ _()` of the Adventure class. We use it there because everytime we create a game, we want it to at least have a list of rooms. Any data an Adventure class needs to function properly is initialized here.
...
To do so, you'll first have to head over to room.py and implement the add_route method for the `Room` class. You might need to add additional attributes to the `_ \_init_ _()` for a proper implementation.
...
Next up is the `move()` method in `Adventure`. We've already initialized the `current_room` attribute in the `_ \_init_ _()` and set it to the room with id 1.
...
You can test moving around by adding the following code to `if _ \_name_ _ == '_ \_main_ _'`:
----
...
Remove the code used to test moving around from the `if _ \_name_ _ == '_ \_main_ _'` and instead add `adventure.play()`.
